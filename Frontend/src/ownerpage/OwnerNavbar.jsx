      ...
      <div className="bg-gray-100 shadow-md fixed top-0 left-0 w-full z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link to="kitchen" className="hover:text-blue-500 transition-colors duration-200 font-semibold">My Kitchen</Link>
            <Link to="additem" className="hover:text-blue-500 transition-colors duration-200 font-semibold">Add Items</Link>
          </div>
          <div className="flex items-center gap-2 md:gap-6">
            ...
            <div className="relative dropdown-menu" ref={dropdownRef}>
              {isMobile ? (
                <button onClick={toggleDropdown} className="p-2 hover:text-blue-500">{isDropdownOpen ? <X size={24} /> : <Menu size={24} />}</button>
              ) : (
                <button onClick={toggleDropdown} className="bg-gray-200 px-3 py-1 rounded-md text-gray-800 font-semibold hover:bg-gray-300">Options</button>
              )}
              {isDropdownOpen && (
